# Evgeni Gomziakov

Backend developer building cloud systems and developer tools.

Over the past 7+ years, I’ve worked across insurance, media measurement, education, and early-stage products. I focus on reliable backend systems and practical tools that help engineering teams move faster.

Currently building backend services at Lemonade.

## Selected Work

- [vibe-shield](https://github.com/gomzkov/vibe-shield)  
  Lightweight security checks for AI-generated JavaScript, TypeScript, and Python projects.

- [what2take](https://what2take.com)  
  An AI-powered packing list with live weather data and shareable trips.

- [my-node-cli](https://github.com/gomzkov/my-node-cli)  
  Practical examples for building interactive command-line applications with Node.js.

## Writing

- [Building a Fully Offline AI App](https://gomzkov.medium.com/how-i-built-a-fully-offline-ai-app-for-private-journaling-25911b448671)
- [Building a CLI App with Node.js](https://gomzkov.medium.com/building-a-cli-with-node-js-in-2024-c278802a3ef5)
- [4 Habits That Made Me a Better Developer](https://gomzkov.medium.com/4-habits-that-made-me-a-better-developer-39b647903534)

## Find Me

[Website](https://gomzkov.com) · [LinkedIn](https://www.linkedin.com/in/gomzkov) · [Medium](https://gomzkov.medium.com) · [X](https://x.com/gomzkov) · [Email](mailto:hello@gomzkov.com)
